...
@Return void
@Param 
This method updates the value of turn_info based on whose turn it is
*/
...
function reset_play(){
	...
	p2.value = "";
	p2.disabled = false;
	ti.innerHTML = "Game has not begin.";
...
function play() {
	if(started){
		move_id = document.getElementById("move_text_id");
		val = move_id.value;
		move_id.value = "";
	
		if(table_ids.includes(val) && board_state[table_ids.indexOf(val)] == -1){
			let mark = "O"
			let mark_int = 0;
			if(whose_move()){
				mark = "X"
			...
			document.getElementById(val).innerHTML = mark;
			board_state[table_ids.indexOf(val)] = mark_int;
			...
			alert("Invalid move, please enter a valid value present in the table. Also make sure the cell hasn't been taken.");
		}
	} else {
...
/*
@Return bool This returns the state of the board game. 
Please note that we return:
true if the game is over (there is a winner)
false if the game is not over (there is not a winner)
@Param - No param
*/
function check_board_win(){
	// Check triplets of the board game

	// Horizontal
	if(	(board_state[0] == board_state[1] && board_state[1] == board_state[2]) && 
		(board_state[0] == 0 || board_state[0] == 1) ){
			return true;
	}

	if(	(board_state[3] == board_state[4] && board_state[4] == board_state[5]) && 
		(board_state[3] == 0 || board_state[3] == 1) ){
			return true;
	}

	if(	(board_state[6] == board_state[7] && board_state[7] == board_state[8]) && 
		(board_state[6] == 0 || board_state[6] == 1) ){
			return true;
	}
	...
	if(	(board_state[1] == board_state[4] && board_state[4] == board_state[7]) && 
		(board_state[1] == 0 || board_state[1] == 1) ){
			return true;
	}

	if(	(board_state[2] == board_state[5] && board_state[5] == board_state[8]) && 
		(board_state[2] == 0 || board_state[2] == 1) ){
			return true;
	}
